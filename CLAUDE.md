...
- [ ] **Phase 7 — Accounts, roles & the visibility spine (enforced).** Login, DM-vs-player permissions, server-side enforcement of public/private knowledge across every view. **This is the leap to a true multi-user web app.**
  ...
  - [ ] **7e — Player read-only views (planned).** Today players are fully blocked from Party/Bestiary/Combat/Loot. Intended refinement: give players a **read-only Combat** view (initiative order, whose turn, their own HP/conditions — DM still runs it) and a **read-only Party** overview (teammates' status; rest/HP stay DM-only). Bestiary stays DM-authoring (player monster knowledge comes via the inspector of *revealed* monsters); Loot stays DM-managed (players already get items on their sheet). Will need read-only variants of those views + relaxing the `_DM_ONLY_ENDPOINTS` gate for the read paths only.
