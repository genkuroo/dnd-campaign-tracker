...
- [x] **Phase 4 — Inventory, loot, currency, XP/leveling.** Migration 5 adds `xp`/`gold`/`silver`/`copper` to creatures; `models/creature.py` has the 5e XP table (`level_from_xp`, `xp_to_next`) — level stays manual (milestone-friendly) with XP-based level-up hints. Migration 6 + `models/inventory.py`: per-creature `creature_items` (name, quantity, description, equipped; ON DELETE CASCADE). Sheet shows a coin purse, XP progress, and an Inventory section with add / qty ± (clamped ≥1) / equip toggle / remove.
