...
- [x] **Phase 2 — Dice roller.** `models/dice.py` parses expressions (`2d6+3`, `1d20+1d4-1`, bounds-guarded) and d20 checks with advantage/disadvantage. Dice tab: quick die buttons (d4–d100) + adv/dis, free-form expression input, and a persisted roll log (migration 3 `rolls` table, `models/roll_log.py`). Wired to the sheet: a per-ability **Check** button rolls `d20 + modifier`, logs it, and flashes the result in place (`next` returns to the sheet; `_safe_next` guards against open redirects). Saving throws fold in once proficiency exists.
