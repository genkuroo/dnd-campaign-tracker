...
- [ ] **Phase 7 — Accounts, roles & the visibility spine (enforced).** Login, DM-vs-player permissions, server-side enforcement of public/private knowledge across every view. **This is the leap to a true multi-user web app.** **DM-only gating chokepoint:** `is_dm()` in `app.py` already guards DM-only actions (returns True pre-auth); making it session/role-aware here locks down everything routed through it at once. **Features marked DM-only so far (must stay DM-only):** the Party tab's per-PC **damage/heal** control and **party rests** (long/short) — both guard the route (403) and hide the UI via `is_dm()`. Audit other DM actions to route through the same helper here (monster create/edit, loot spawn/give, area + encounter management, combat mutations). Also lands **per-player roll colors**: rolls stay server-side/authoritative (anti-cheat, shared log); once players have identity, each gets a color and the dice log tints rolls by roller. (Deliberately deferred here rather than a pre-auth per-session hack.) **Open question to resolve here:** spellbook/ability/inventory visibility — default is *creature-level* (a hidden creature hides its whole stat block), vs optional *per-entry reveal* (fog-of-war on individual spells/items, like the DM-gated inspector). Decided to defer the choice until this phase; if per-entry, add a `visibility` column to `creature_spells`/items then.
