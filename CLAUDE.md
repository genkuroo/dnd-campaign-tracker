...
- [ ] **Phase 7 — Accounts, roles & the visibility spine (enforced).** Login, DM-vs-player permissions, server-side enforcement of public/private knowledge across every view. **This is the leap to a true multi-user web app.**
  ...
  - [ ] **7c — Visibility enforcement.** Players see only their own character + `visibility='visible'` NPCs/monsters/entities (the fog-of-war spine); DM sees all. Server-side filters across roster, bestiary, inspector, loot, etc. Also gate remaining DM actions through `is_dm()` (monster create/edit, loot spawn/give, area + encounter management, combat mutations).
